...
$borrow_records_sql = "SELECT b.borrow_id, b.user_id, u.username, b.item_id, b.category, b.quantity, b.borrow_date, b.status,
                                CASE 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    foreach ($_POST['requests'] as $borrow_id => $action) {
        if ($action == 'approve' || $action == 'reject') {
            ...
            $update_stmt->bind_param("si", $new_status, $borrow_id);
            $update_stmt->execute();
            $update_stmt->close();
        }
    }
